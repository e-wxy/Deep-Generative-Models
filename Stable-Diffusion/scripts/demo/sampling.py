...
def load_img(W, H, display=True, key=None, device="cuda"):
...
def run_txt2img(
    ...
    if version.startswith("SDXL-base"):
        W, H = st.selectbox("Resolution:", list(SD_XL_BASE_RATIOS.values()), 4)
    else:
...
if __name__ == "__main__":
    ...
    version = 'SDXL-refiner-1.0'
    # version = 'SDXL-base-1.0'
    # version = st.selectbox("Model Version", list(VERSION2SPECS.keys()), 0)
    version_dict = VERSION2SPECS[version]
    # if st.checkbox("Load Model"):
    #     mode = st.radio("Mode", ("txt2img", "img2img"), 1)
    # else:
    #     mode = "skip"
    mode = st.radio("Mode", ("txt2img", "img2img"), 1)
    ...
    prompt = st.text_input(
        ...
        "Painting in Monet's style, muted colors, Impressionistic strokes, detailed, 8k",
    )
    if is_legacy:
    ...
    if isinstance(out, (tuple, list)):
        ...
        del out  # Clear tensor from memory
        torch.cuda.empty_cache()
        gc.collect()
    else:
        samples = out
        samples_z = None
        del out  # Clear image tensor from memory
        torch.cuda.empty_cache()
        gc.collect()
    ...
    if save_locally and samples is not None:
        ...
        del samples, samples_z  # Clear image tensor from memory
        torch.cuda.empty_cache()
        gc.collect()